...
> - **Planned (later phases):** synchronous quorum write replication, request
>   routing (reads to the cache owner, writes to the coordinator), a replication
>   log + anti-entropy catch-up, ownership handoff on failure, and distributed
>   transactions.
>
...
## Write path & quorum (planned)

The intended write path: the owner acquires the collection write lock, commits
locally, appends to a replication log, and replicates to peers, returning `OK` only
once a **majority** has acknowledged (`replicationAckTimeoutMs` bounds the wait).
`OwnershipManager.hasQuorum()` computes the majority as
`⌊max(clusterExpectedSize, knownMembers) / 2⌋ + 1`. The quorum gate is what makes
ownership handoff safe: a node in a **minority partition** cannot reach a majority,
so it cannot commit divergent writes (it will refuse writes and serve reads only).
...
in-flight requests. Inbound messages whose `secret` does not match `clusterSecret`
are rejected.
