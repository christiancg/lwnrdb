...
Conformance is **measured, not asserted** (currently **99.92%**, 36,449/36,477). The denominator
...
| 7 | **The Unicode version is the build JDK's, not a pinned one** | A conformant engine pins the UCD version the spec requires; with no ICU dependency ours is whatever `java.util.regex` ships — Unicode 16.0 on JDK 25, 17.0 on JDK 26. So `\p{…}` escapes answer differently across the two supported build JDKs for anything added in 17.0, and on JDK 25 the scripts added there (`Sidetic`, `Tolong_Siki`, `Tai_Yo`, `Beria_Erfe`) throw a `SyntaxError` instead of compiling. This is why `built-ins/RegExp/property-escapes/generated/` is excluded from the gate rather than baselined — see `config/test262-exclusions.txt`. **Properties of strings** (`\p{RGI_Emoji}` and its six siblings) are the one exception: they are sets of *strings*, so the JDK has no data for them at all, and `RegexTranslator` expands each into an alternation of literal sequences from `src/main/resources/simplejs/emoji-sequences.txt` — data that is **ours and pinned to Unicode 17.0** (regenerate with `test_utils/gen_emoji_sequences.py`, which reads the UTS #51 sequence files). These escapes therefore answer identically on JDK 25 and 26, unlike every other `\p{…}`; the price is that the table must be regenerated when the pinned corpus moves to a new UCD. Property names are now matched **exactly** — loose matching (`\p{ gc = X }`) is an early error — and the `Hex` alias is supported. |
| 8 | **A capturing group inside a repeated group is not reset between iterations** | The spec's backtracking semantics reset every capture inside a quantified body at the start of each iteration attempt (and discard captures made by an assertion, e.g. a negative lookahead, that ends up not "winning"); `java.util.regex`'s `Matcher` keeps whatever a capturing group last matched until that same numbered group matches again, so a group that only participated in an *earlier* iteration (or only inside a negative lookahead that correctly failed to match) still reports that stale value instead of `undefined`. This is a `Matcher`-level limitation, not a translation issue — no `RegexTranslator` rewrite of the pattern text changes how `Matcher` tracks group state across backtracking — and it is what the sputnik `(z)((a+)?(b+)?(c))*` family, the duplicate-named-group-under-a-quantifier tests, `S15.10.2.8_A2_T1` (`\2` after a failed `(?!(a+)b\2c)`), and the "nullable quantifier"/zero-length-iteration tests are really exercising. Fixing it in general needs a backtracking matcher that snapshots and restores capture state per attempt, which `java.util.regex` does not expose and this codebase does not implement. |
| 9 | **A lookbehind containing a backreference is rejected at compile time** | `java.util.regex` requires a lookbehind's body to have a statically-computable maximum length so it can implement lookbehind as "scan backward that far, then match forward"; a backreference's matched length is not known until match time, so `Pattern.compile` throws `PatternSyntaxException: Look-behind group does not have an obvious maximum length` for any lookbehind whose body contains one (`(?<=(\w+)\1)`, mutually-recursive backreferences, etc). A real backtracking engine has no such restriction (it walks the lookbehind body backward the same way it walks any other alternative). There is no lookbehind-specific workaround: the pattern cannot be rewritten to give the backreference a bounded length without changing what it matches. |
| 10 | **Without `/u` or `/v`, `.` and character classes still combine a well-formed surrogate pair into one code point** | Per spec, without `/u`/`/v` a pattern matches **code units**: `.` (and any character class) should consume exactly one UTF-16 unit, even mid-string where the next unit happens to complete a valid surrogate pair. `java.util.regex`'s class/dot nodes always decode via `Character.codePointAt`, which combines a well-formed lead+trail surrogate pair into one supplementary code point before testing it against the class — confirmed (see the investigation in `RegexTranslatorTest`/this file's history) to hold regardless of how the class is spelled: a plain negated class, an explicit BMP-only intersection, `\uXXXX` code-unit escapes and `\x{…}` codepoint escapes all still combine the pair when it is well-formed, and there is no `Pattern` flag or class construction that reverts to per-code-unit stepping. This is what `dotall/without-dotall.js`, the `regexp-modifiers` dotAll-interaction tests, `match-indices/indices-array-non-unicode-match.js`, `RegExp.prototype[Symbol.match]` unicode-inference, and the non-`/u` half of `regexp-prototype-match-v-u-flag.js` are exercising; it needs a matcher that steps by code unit instead of by code point when `/u`/`/v` are absent. |
