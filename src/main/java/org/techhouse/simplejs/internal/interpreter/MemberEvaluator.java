...
public final class MemberEvaluator {
    ...
    public JsValue getMember(JsValue target, String key) {
        return switch (target) {
            ...
            case JsTemporalInstant instant -> intrinsicMember(instant, key);
            case JsTemporalPlainYearMonth yearMonth -> intrinsicMember(yearMonth, key);
            case JsTemporalPlainMonthDay monthDay -> intrinsicMember(monthDay, key);
            case JsTemporalPlainDateTime dt -> intrinsicMember(dt, key);
            case JsTemporalZonedDateTime zdt -> intrinsicMember(zdt, key);
            case JsGeo geo -> intrinsicMember(geo, key);
            case JsVector vector -> intrinsicMember(vector, key);
            case JsDbDateTime dbDateTime -> intrinsicMember(dbDateTime, key);
            case JsDbTime dbTime -> intrinsicMember(dbTime, key);
