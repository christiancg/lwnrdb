 ...
 *   }
 * }</pre>
 * But EJson is unable to deserialize this value because the JSON string name is
 * just the {@link Object#toString() toString()} of the map key. Attempting to
 * convert the above JSON to an object fails with a parse exception:
 * <pre>JsonParseException: Expecting object found: "(5,6)"
 *   at JsonObjectDeserializationVisitor.visitFieldUsingCustomHandler
 *   at ObjectNavigator.navigateClassFields
 *   ...</pre>
 *
 ...
 *
 * <p>Register this adapter when you are creating your EJson instance.
 * <pre>   {@code
...
public final class MapTypeAdapterFactory implements TypeAdapterFactory {
  ...
  @Override public <T> TypeAdapter<T> create(EJson eJson, TypeToken<T> typeToken) {
    ...
    Type[] keyAndValueTypes = $EJson$Types.getMapKeyAndValueTypes(type, rawType);
    TypeAdapter<?> keyAdapter = getKeyAdapter(eJson, keyAndValueTypes[0]);
    TypeAdapter<?> valueAdapter = eJson.getAdapter(TypeToken.get(keyAndValueTypes[1]));
    ObjectConstructor<T> constructor = constructorConstructor.get(typeToken);

    @SuppressWarnings({"unchecked", "rawtypes"})
    // we don't define a type parameter for the key or value types
    TypeAdapter<T> result = new Adapter(eJson, keyAndValueTypes[0], keyAdapter,
   ...
   */
  private TypeAdapter<?> getKeyAdapter(EJson context, Type keyType) {
  ...
  private final class Adapter<K, V> extends TypeAdapter<Map<K, V>> {
    ...
    public Adapter(EJson context, Type keyType, TypeAdapter<K> keyTypeAdapter,
