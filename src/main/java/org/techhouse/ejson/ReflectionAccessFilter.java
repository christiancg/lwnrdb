...
import org.techhouse.ejson.internal.ReflectionAccessFilterHelper;
 ...
 * fields and classes.
 *
 * @see EJsonBuilder#addReflectionAccessFilter(ReflectionAccessFilter)
 * @since 2.9.1
 */
public interface ReflectionAccessFilter {
  ...
  enum FilterResult {
     ...
     * it only permits EJson to try using reflection for a class. The Java
     ...
     * restrictions imposed by a SecurityManager are not considered.
     *
   ...
   */
  ReflectionAccessFilter BLOCK_INACCESSIBLE_JAVA = rawClass -> ReflectionAccessFilterHelper.isJavaType(rawClass)
   ...
   */
  ReflectionAccessFilter BLOCK_ALL_JAVA = rawClass -> ReflectionAccessFilterHelper.isJavaType(rawClass)
   ...
   * @see FilterResult#BLOCK_ALL
   */
  ReflectionAccessFilter BLOCK_ALL_ANDROID = rawClass -> ReflectionAccessFilterHelper.isAndroidType(rawClass)
    ? FilterResult.BLOCK_ALL
    : FilterResult.INDECISIVE;
   ...
   * @see FilterResult#BLOCK_ALL
   */
  ReflectionAccessFilter BLOCK_ALL_PLATFORM = rawClass -> ReflectionAccessFilterHelper.isAnyPlatformType(rawClass)
    ? FilterResult.BLOCK_ALL
    : FilterResult.INDECISIVE;
