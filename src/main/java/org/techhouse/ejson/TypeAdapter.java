...
import org.techhouse.ejson.internal.bind.JsonTreeReader;
import org.techhouse.ejson.internal.bind.JsonTreeWriter;
 ...
 * By default EJson converts application classes to JSON using its built-in type
 * adapters. If EJson's default JSON conversion isn't appropriate for a type,
 ...
 *     }
 *   }}</pre>
 * With this type adapter installed, EJson will convert {@code Points} to JSON as
 ...
 * registering the type adapter with EJson. If your {@code EJson} instance
 * has been configured to {@link EJsonBuilder#serializeNulls()}, these nulls will be
 ...
 *
 * <p>To use a custom type adapter with EJson, you must <i>register</i> it with a
 * {@link EJsonBuilder}: <pre>{@code
 *   EJsonBuilder builder = new EJsonBuilder();
 *   builder.registerTypeAdapter(Point.class, new PointAdapter());
 *   // if PointAdapter didn't check for nulls in its read/write methods, you should instead use
 *   // builder.registerTypeAdapter(Point.class, new PointAdapter().nullSafe());
 *   ...
 *   EJson eJson = builder.create();
 * }</pre>
 *
 * @since 2.1
 */
...
// <p>A type adapter registered with EJson is automatically invoked while serializing
// or deserializing JSON. However, you can also use type adapters directly to serialize
// and deserialize JSON. Here is an example for deserialization: <pre>{@code
//   String json = "{'origin':'0,0','points':['1,2','3,4']}";
//   TypeAdapter<Graph> graphAdapter = eJson.getAdapter(Graph.class);
...
public abstract class TypeAdapter<T> {
   ...
   * <pre>{@code
   * EJson eJson = new EJsonBuilder().registerTypeAdapter(Foo.class,
   *   new TypeAdapter<Foo>() {
