package org.techhouse.ejson.reflect;

import org.techhouse.ejson.internal.$EJson$Types;
import org.techhouse.ejson.internal.TroubleshootingGuide;
 ...
 * to EJson and therefore it cannot provide the functionality one might expect.
...
public class TypeToken<T> {
  ...
  protected TypeToken() {
    ...
    this.rawType = (Class<? super T>) $EJson$Types.getRawType(type);
  ...
  private TypeToken(Type type) {
    this.type = $EJson$Types.canonicalize(Objects.requireNonNull(type));
    this.rawType = (Class<? super T>) $EJson$Types.getRawType(this.type);
    this.hashCode = this.type.hashCode();
  }

  private static boolean isCapturingTypeVariablesForbidden() {
    return !Objects.equals(System.getProperty("eJson.allowCapturingTypeVariables"), "true");
  }

  /**
   * Verifies that {@code this} is an instance of a direct subclass of TypeToken and
   * returns the type argument for {@code T} in {@link $EJson$Types#canonicalize
   * canonical form}.
   */
  private Type getTypeTokenTypeArgument() {
    ...
    if (superclass instanceof ParameterizedType parameterized) {
      if (parameterized.getRawType() == TypeToken.class) {
        Type typeArgument = $EJson$Types.canonicalize(parameterized.getActualTypeArguments()[0]);
  ...
  private static void verifyNoTypeVariable(Type type) {
      switch (type) {
          ...
          case GenericArrayType genericArrayType -> verifyNoTypeVariable(genericArrayType.getGenericComponentType());
  ...
  public boolean isAssignableFrom(Type from) {
    ...
    if (type.equals(from)) {
      ...
      return switch (type) {
          case Class<?> ignored -> rawType.isAssignableFrom($EJson$Types.getRawType(from));
          case ParameterizedType parameterizedType -> isAssignableFrom(from, parameterizedType,
                  new HashMap<>());
          case GenericArrayType genericArrayType -> rawType.isAssignableFrom($EJson$Types.getRawType(from))
                  && isAssignableFrom(from, genericArrayType);
          case null, default -> throw buildUnexpectedTypeError(
  ...
  private static boolean isAssignableFrom(Type from, ParameterizedType to,
    ...
    Class<?> clazz = $EJson$Types.getRawType(from);
  ...
  @Override public final boolean equals(Object o) {
    return o instanceof TypeToken<?>
        && $EJson$Types.equals(type, ((TypeToken<?>) o).type);
  }

  @Override public final String toString() {
    return $EJson$Types.typeToString(type);
  }
  ...
  public static TypeToken<?> getParameterized(Type rawType, Type... typeArguments) {
    ...
    if ($EJson$Types.requiresOwnerType(rawType)) {
    ...
    for (int i = 0; i < expectedArgsCount; i++) {
      ...
      Class<?> rawTypeArgument = $EJson$Types.getRawType(typeArgument);
      TypeVariable<?> typeVariable = typeVariables[i];

      for (Type bound : typeVariable.getBounds()) {
        Class<?> rawBound = $EJson$Types.getRawType(bound);
    ...
    return new TypeToken<>($EJson$Types.newParameterizedTypeWithOwner(null, rawType, typeArguments));
  }
  ...
  public static TypeToken<?> getArray(Type componentType) {
    return new TypeToken<>($EJson$Types.arrayOf(componentType));
  }
}
