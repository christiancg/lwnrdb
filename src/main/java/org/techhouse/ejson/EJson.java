...
import org.techhouse.ejson.internal.bind.*;
...
import org.techhouse.ejson.stream.MalformedJsonException;
...
/**
 * This is the main class for using EJson. EJson is typically used by first constructing a
 * EJson instance and then invoking {@link #toJson(Object)} or {@link #fromJson(String, Class)}
 * methods on it. EJson instances are Thread-safe so you can reuse them freely across multiple
 * threads.
 *
 * <p>You can create a EJson instance by invoking {@code new EJson()} if the default configuration
 * is all you need. You can also use {@link EJsonBuilder} to build a EJson instance with various
 ...
 *
 * <p>Here is an example of how EJson is used for a simple Class:
 *
 * <pre>
 * EJson eJson = new EJson(); // Or use new EJsonBuilder().create();
 * MyType target = new MyType();
 * String json = eJson.toJson(target); // serializes target to JSON
 * MyType target2 = eJson.fromJson(json, MyType.class); // deserializes json into target2
 * </pre>
 *
 ...
 * or {@code TypeToken} is optional, otherwise EJson will use the runtime type of the object.
 * {@link TypeToken} is a class provided by EJson which helps creating parameterized types.
 ...
 * // For serialization you normally do not have to specify the type, EJson will use
 * // the runtime type of the objects, however you can also specify it explicitly
 * String json = eJson.toJson(target, listType.getType());
 *
 * // But for deserialization you have to specify the type
 * List&lt;MyType&gt; target2 = eJson.fromJson(json, listType);
 * </pre>
 *
 * <p>See the <a href="https://github.com/google/eJson/blob/main/UserGuide.md">EJson User Guide</a>
 * for a more complete set of examples.</p>
 *
 * <h2 id="default-lenient">JSON Strictness handling</h2>
 * For legacy reasons most of the {@code EJson} methods allow JSON data which does not
 * comply with the JSON specification when no explicit {@linkplain Strictness strictness} is set (the default).
 * To specify the strictness of a {@code EJson} instance, you should set it through
 * {@link EJsonBuilder#setStrictness(Strictness)}.
 *
 * <p>For older EJson versions, which don't have the strictness mode API, the following
 ...
 *       {@code EJson} instance listed by {@link #newJsonWriter(Writer)}.<br>
 ...
 *       {@code EJson} instance listed by {@link #newJsonReader(Reader)}.<br>
...
public final class EJson {
  ...
  // Strictness of `null` is the legacy mode where some EJson APIs are always lenient
  ...
  /**
   * Constructs a EJson object with default configuration. The default configuration has the
   * following settings:
   * <ul>
   *   <li>The JSON generated by <code>toJson</code> methods is in compact representation. This
   *   means that all the unneeded white-space is removed. You can change this behavior with
   *   {@link EJsonBuilder#setPrettyPrinting()}.</li>
   *   <li>When the JSON generated contains more than one line, the kind of newline and indent to
   *   use can be configured with {@link EJsonBuilder#setFormattingStyle(FormattingStyle)}.</li>
   *   <li>The generated JSON omits all the fields that are null. Note that nulls in arrays are
   *   kept as is since an array is an ordered list. Moreover, if a field is not null, but its
   *   generated JSON is empty, the field is kept. You can configure EJson to serialize null values
   *   by setting {@link EJsonBuilder#serializeNulls()}.</li>
   *   <li>EJson provides default serialization and deserialization for Enums, {@link Map},
   *   {@link java.net.URL}, {@link java.net.URI}, {@link java.util.Locale}, {@link java.util.Date},
   *   {@link BigDecimal}, and {@link BigInteger} classes. If you would prefer
   *   to change the default representation, you can do so by registering a type adapter through
   *   {@link EJsonBuilder#registerTypeAdapter(Type, Object)}. </li>
   *   <li>The default Date format is same as {@link DateFormat#DEFAULT}. This format
   *   ignores the millisecond portion of the date during serialization. You can change
   *   this by invoking {@link EJsonBuilder#setDateFormat(int)} or
   *   {@link EJsonBuilder#setDateFormat(String)}. </li>
   *   <li>The default field naming policy for the output JSON is same as in Java. So, a Java class
   *   field <code>versionNumber</code> will be output as <code>&quot;versionNumber&quot;</code> in
   *   JSON. The same rules are applied for mapping incoming JSON to the Java classes. You can
   *   change this policy through {@link EJsonBuilder#setFieldNamingPolicy(FieldNamingPolicy)}.</li>
   *   <li>By default, EJson excludes <code>transient</code> or <code>static</code> fields from
   *   consideration for serialization and deserialization. You can change this behavior through
   *   {@link EJsonBuilder#excludeFieldsWithModifiers(int...)}.</li>
   *   <li>No explicit strictness is set. You can change this by calling
   *   {@link EJsonBuilder#setStrictness(Strictness)}.</li>
   * </ul>
   */
  public EJson() {
  ...
  EJson(Excluder excluder, FieldNamingStrategy fieldNamingStrategy,
        Map<Type, InstanceCreator<?>> instanceCreators, boolean serializeNulls,
        boolean complexMapKeySerialization, boolean generateNonExecutableEJson, boolean htmlSafe,
    ...
    this.generateNonExecutableJson = generateNonExecutableEJson;
    ...
    // Add adapter for LazilyParsedNumber because user can obtain it from EJson and then try to serialize it again
    ...
    factories.add(new ReflectiveTypeAdapterFactory(
        constructorConstructor, fieldNamingStrategy, excluder, reflectionFilters));

    this.factories = Collections.unmodifiableList(factories);
  }

  /**
   * Returns a new EJsonBuilder containing all custom factories and configuration used by the current
   * instance.
   *
   * @return a EJsonBuilder instance.
   * @since 2.8.3
   */
  public EJsonBuilder newBuilder() {
    return new EJsonBuilder(this);
  }

  /**
   * @deprecated This method by accident exposes an internal EJson class; it might be removed in a
   * future version.
   */
  @Deprecated
  ...
  /**
   * Returns the field naming strategy used by this EJson instance.
   *
   * @see EJsonBuilder#setFieldNamingStrategy(FieldNamingStrategy)
   */
  ...
  /**
   * Returns whether this EJson instance is serializing JSON object properties with
   * {@code null} values, or just omits them.
   *
   * @see EJsonBuilder#serializeNulls()
   */
  ...
  /**
   * Returns whether this EJson instance produces JSON output which is
   * HTML-safe, that means all HTML characters are escaped.
   *
   * @see EJsonBuilder#disableHtmlEscaping()
   */
  ...
  static void checkValidFloatingPoint(double value) {
    if (Double.isNaN(value) || Double.isInfinite(value)) {
      throw new IllegalArgumentException(value
          ...
          + " behavior, use EJsonBuilder.serializeSpecialFloatingPointValues() method.");
    }
  }
   ...
   *
   * @throws IllegalArgumentException if this EJson instance cannot serialize and
  ...
  public <T> TypeAdapter<T> getAdapter(TypeToken<T> type) {
    ...
    if (candidate == null) {
      throw new IllegalArgumentException("EJson cannot handle " + type);
    }
    ...
    if (isInitialAdapterRequest) {
       ...
       * See https://github.com/google/eJson/issues/625
       */
   ...
   * adapter that does a little bit of work but then delegates further processing to the EJson
   ...
   *     final TypeAdapter<T> delegate = eJson.getDelegateAdapter(this, type);
   ...
   * EJson eJson = new EJsonBuilder().registerTypeAdapterFactory(stats).create();
   * // Call eJson.toJson() and fromJson methods on objects
   ...
   * {@link EJsonBuilder#registerTypeAdapter(Type, Object)}, our stats factory will not count
   * the number of instances of those types that will be read or written.
   *
   * <p>If {@code skipPast} is a factory which has not been registered on the {@link EJsonBuilder} then this
   * method behaves as if {@link #getAdapter(TypeToken)} had been called.
   *
  ...
  public <T> TypeAdapter<T> getDelegateAdapter(TypeAdapterFactory skipPast, TypeToken<T> type) {
    ...
    if (skipPastFound) {
      throw new IllegalArgumentException("EJson cannot serialize " + type);
    } else {
   ...
   * {@code writer}.
   *
   * <p>If the {@code EJson} instance has an {@linkplain EJsonBuilder#setStrictness(Strictness) explicit strictness setting},
   * this setting will be used for writing the JSON regardless of the {@linkplain JsonWriter#getStrictness() strictness}
   * of the provided {@link JsonWriter}. For legacy reasons, if the {@code EJson} instance has no explicit strictness setting
   ...
   *
   * <p>The 'HTML-safe' and 'serialize {@code null}' settings of this {@code EJson} instance
   * (configured by the {@link EJsonBuilder}) are applied, and the original settings of the
  ...
  /**
   * Returns a new JSON writer configured for the settings on this EJson instance.
   *
   * <p>The following settings are considered:
   * <ul>
   *   <li>{@link EJsonBuilder#disableHtmlEscaping()}</li>
   *   <li>{@link EJsonBuilder#generateNonExecutableJson()}</li>
   *   <li>{@link EJsonBuilder#serializeNulls()}</li>
   *   <li>{@link EJsonBuilder#setStrictness(Strictness)}. If no
   *   {@linkplain EJsonBuilder#setStrictness(Strictness) explicit strictness has been set} the created
   *   writer will have a strictness of {@link Strictness#LEGACY_STRICT}. Otherwise, the strictness of
   *   the {@code EJson} instance will be used for the created writer.</li>
   *   <li>{@link EJsonBuilder#setPrettyPrinting()}</li>
   *   <li>{@link EJsonBuilder#setFormattingStyle(FormattingStyle)}</li>
   * </ul>
   */
  ...
  /**
   * Returns a new JSON reader configured for the settings on this EJson instance.
   *
   * <p>The following settings are considered:
   * <ul>
   *   <li>{@link EJsonBuilder#setStrictness(Strictness)}. If no
   ...
   *   the {@code EJson} instance will be used for the created reader.</li>
   * </ul>
   */
  ...
  // source compatibility in certain cases, see https://github.com/google/eJson/pull/1700#discussion_r973764414
   ...
   * of the provided {@link JsonReader}. For legacy reasons, if the {@code EJson} instance has no explicit strictness setting
   ...
   * has been set.
   *
   * @see EJson#threadLocalAdapterResults
   */
